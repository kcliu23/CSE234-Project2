...
\texttt{data\_prep/} holds the data-side scripts: \texttt{augment\_data.py}
(SBO augmentation via Claude Haiku 4.5, 9 batches, each generated SQL
validated through \texttt{sql\_to\_schema\_links.py}, audit log preserved),
\texttt{format\_training\_data.py} and \texttt{format\_two\_stage\_data.py}
(JSON $\rightarrow$ JSONL conversion for TRL/RapidFire and the two-stage
experiment), \texttt{generate\_cot\_traces.py} (Haiku-generated rationales
for sweep 21's CoT-distillation experiment), and \texttt{expand\_columns.py}
(the post-processor in Sec.~4). \texttt{training/reproduce\_champion.py}
rebuilds the 4-way + \texttt{maj2} predictions from the per-(adapter,
retriever) prediction files on disk -- no model loads.
...
(combo, aggregation) cells, calls \texttt{eval.py} on each, ranks by score.
\texttt{logs/README.md} maps each sweep number to its config, status, and
best train metric, built from the rapidfireai MLflow store.
...
\textbf{Surprises.} \texttt{types\_keys} prompts hurt (richer metadata made
the model less discriminative) and \texttt{max\_tables=40} hurt ($-$0.02)
-- on a $\sim$1.5B model with a small training set, \emph{less context is
more}.

\textbf{What we would try next.} (1)~Retrain on embed-filtered prompts to
close the small train/test retrieval-distribution gap (+0.005--0.02
expected). (2)~Longer CoT traces ($\sim$200 tokens) with explicit
sibling-table disambiguation. (3)~Hybrid retrieval with learned fusion
weights and a small cross-encoder reranker over the union of BM25 + embed
top-30 candidates.
