...
\textbf{Final validation leaderboard: 0.7107} (Table 0.7791 / Column 0.6423).
...
\emph{across two base models AND mixed table retrievers per adapter}: sweep 13
on Qwen-Coder-1.5B with embedding retrieval, sweep 15 on Qwen-Coder-1.5B with
BM25 retrieval, sweep 22 on Qwen3-1.7B with embedding retrieval. Completes
101 questions in \textbf{12m14s} on a 24~GB MIG slice ($<$15~min budget).
...
\texttt{path[:retriever]} spec per adapter so the same adapter under different
retrievers can be ensembled too.
...
via \texttt{PeftModel.load\_adapter}. (3)~For each adapter in the group:
\texttt{model.set\_adapter(name)}, run all 101 queries through that adapter,
accumulate $\{$table$\rightarrow$cols$\}$ predictions. (4)~Free the group's
base before loading the next group. (5)~Union the accumulator across all
adapters and write final predictions. \textbf{Partial-save after each
adapter}: a $>$15min timeout still yields a smaller ensemble of completed
adapters. \textbf{Retrieval}: embedding-based --
...
column-tokens}'' document; top-20 tables kept.
...
\multicolumn{6}{r}{\textbf{3-way mixed-retriever: 13\_embed + 15\_BM25 + 22\_embed}}
                                                              & \textbf{0.7107} \\
\multicolumn{6}{r}{(submitted champion; 0.7147 in the offline probe -- the 0.7107 is the actual fp16 end-to-end)} & \\
\bottomrule
\end{tabular}
...
boost: the 3-way mixed-retriever ensemble \textbf{13\_embed + 15\_BM25 +
22\_embed lands at 0.7107} -- our final submitted leaderboard. The
intuition: ``which retriever is right for this adapter'' is not just about
training-time consistency; for ensemble purposes, the right pairing is
whichever makes the errors of this member most uncorrelated with the
others. We expose this in \texttt{main.py}'s \texttt{--ensemble\_dirs}
via a \texttt{path[:retriever]} spec per adapter.
\end{itemize}
